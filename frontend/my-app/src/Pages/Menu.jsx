...
const RatingModal = ({ dish, onClose, dark, getAuthHeader }) => {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    try {
      // ✅ api.js: 60s timeout + auto-attach token
      await api.post("/api/v1/ratings/", { dish_id: dish._id, stars, comment });
...
const Menu = () => {
  ...
  // ✅ FIXED: was raw fetch() with no timeout — now uses api.get() with 60s timeout
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        const res = await api.get("/api/v1/menu");
        setDishes(Array.isArray(res.data) ? res.data : res.data.menu || []);
      } catch {
        setDishes([]);
      } finally {
        setLoading(false);
  ...
  // ✅ FIXED: was raw fetch() — now api.get() with 60s timeout
  useEffect(() => {
  ...
  // ✅ FIXED: was raw axios.get() — now api.get() with 60s timeout
  const fetchRatings = useCallback(async (dishIds) => {
    const results = await Promise.allSettled(
      dishIds.map(id => api.get(`/api/v1/ratings/${id}`))
    );
    const map = {};
    results.forEach((r, i) => {
    ...
    <>
      ...
      <div style={{ padding: "80px 20px 120px", backgroundColor: bg, minHeight: "100vh", fontFamily: "Oswald, sans-serif", color: text }}>
        ...
        <button onClick={toggle} style={{ position: "fixed", top: 20, right: 100, background: dark ? "#e91e63" : "#333", color: "#fff", border: "none", borderRadius: 30, padding: "10px 16px", cursor: "pointer", zIndex: 1000, display: "flex", alignItems: "center", gap: 6, fontFamily: "Oswald, sans-serif", fontSize: 14 }}>
          {dark ? <FaSun size={14} /> : <FaMoon size={14} />} {dark ? "Light" : "Dark"}
        </button>
        ...
        <div style={{ position: "relative", maxWidth: 600, margin: "0 auto 20px" }}>
          ...
          <input type="text" placeholder="Search dishes, categories..." value={search} onChange={e => setSearch(e.target.value)}
                ...
                <div key={dish._id} style={cardStyle(dark)}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                      ...
                      {user && (
                        <button onClick={() => setRatingDish(dish)} style={{ fontSize: 11, color: "#e91e63", background: "none", border: "none", cursor: "pointer", padding: "0 4px", fontFamily: "Oswald, sans-serif", textDecoration: "underline" }}>Rate</button>
                      )}
                    </div>
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8, marginTop: 32 }}>
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)} style={{ ...paginationBtn(dark), background: p === page ? "#e91e63" : dark ? "#16213e" : "#fff", color: p === page ? "#fff" : dark ? "#aaa" : "#555", borderColor: p === page ? "#e91e63" : dark ? "#333" : "#ddd" }}>{p}</button>
            ))}
      ...
      {ratingDish && (
        <RatingModal dish={ratingDish} dark={dark} getAuthHeader={getAuthHeader}
          onClose={(refresh) => { setRatingDish(null); if (refresh) fetchRatings(paginated.map(d => d._id)); }}
        />
      )}
