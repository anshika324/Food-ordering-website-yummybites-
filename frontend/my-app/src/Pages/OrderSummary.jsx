...
import { FaHome, FaCheckCircle, FaClock, FaTruck, FaUtensils, FaTimesCircle, FaBox } from "react-icons/fa";
import CartIcon from "../Components/CartIcon";
import { useTheme } from "../context/ThemeContext";

// ✅ WS_URL derived from BACKEND_URL imported from api.js (single source of truth)
      ...
      {STATUS_STEPS.map((step, idx) => {
        ...
        return (
          <div key={step.key} style={styles.timelineItem}>
            {idx > 0 && <div style={{ ...styles.connector, background: done || active ? "#e91e63" : dark ? "#444" : "#e0e0e0" }} />}
            <div style={{ ...styles.iconCircle, background: done || active ? "#e91e63" : dark ? "#333" : "#f0f0f0", color: done || active ? "#fff" : dark ? "#666" : "#bbb", boxShadow: active ? "0 0 0 5px rgba(233,30,99,0.2)" : "none", transform: active ? "scale(1.18)" : "scale(1)" }}>
              {step.icon}
            </div>
...
const OrderSummary = () => {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      // ✅ api.js: 60s timeout + retry — was plain axios with no timeout
      const res = await api.get(`/api/v1/order/${order_id}`);
      setOrder(res.data);
    } catch (err) {
  ...
  // ✅ WebSocket unchanged — correct WS_URL derivation kept
  useEffect(() => {
    const connect = () => {
      ...
      ws.onclose = () => {
        setWsStatus("offline");
        setTimeout(() => { if (wsRef.current?.readyState !== WebSocket.OPEN) connect(); }, 5000);
      };
    };
